...
function formatAfkDuration(since) { const elapsedMs = Math.max(0, Date.now() - since); const minutes = Math.floor(elapsedMs / 60000); if (minutes < 1) return 'menos de 1 minuto'; if (minutes === 1) return '1 minuto'; if (minutes < 60) return `${minutes} minutos`; const hours = Math.floor(minutes / 60); const remaining = minutes % 60; if (remaining === 0) return hours === 1 ? '1 hora' : `${hours} horas`; return `${hours}h ${remaining}min`; }
...
async function handleAfk(sock, message, effectiveSender, sender, pairingPhone, isGroup, reply, autoDisable = true) { if (autoDisable && !message.key.fromMe) { const ownAfk = findAfkEntry(getAfkKeys({ effectiveSender, sender, pairingPhone, sock })); if (ownAfk) { clearAfk(ownAfk.key); await reply(`👋 @${effectiveSender.split('@')[0]} saiu do AFK!\n⏱️ Tempo ausente: ${formatAfkDuration(ownAfk.entry.since)}\n📝 Motivo: ${ownAfk.entry.reason}`, { mentions: [effectiveSender] }); } } if (!isGroup) return; const mentioned = [...new Set(getMentionedJids(message))]; if (!mentioned.length) return; const notices = []; const mentions = []; for (const jid of mentioned) { const entry = getAfk(jid); if (!entry) continue; notices.push(`💤 @${jid.split('@')[0]} está AFK.\n📝 Motivo: ${entry.reason}\n⏱️ Ausente há ${formatAfkDuration(entry.since)}`); mentions.push(jid); } if (notices.length) await reply(`╭━━━〔 💤 𝐀𝐅𝐊 〕━━━╮\n${notices.join('\n\n')}\n╰━━━━━━━━━━━━━━━━━━╯`, { mentions }); }
function isMenuCommand(name) { const normalized = String(name || '').toLowerCase(); return normalized === 'menu' || normalized === 'help' || normalized === 'ajuda' || normalized === 'm' || normalized.startsWith('menu'); }
async function sendMenuReply(sock, chat, message, imageUrl, content, options = {}) { const payload = { image: { url: imageUrl }, caption: String(content), ...options }; if (message.key.fromMe) return sock.sendMessage(chat, payload); return sock.sendMessage(chat, payload, { quoted: message }); }
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    if (!state.creds.registered && pairingPhone && !pairingRequested && qr) { pairingRequested = true; try { const code = await sock.requestPairingCode(pairingPhone); logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`); logger.info(`🔐 TOGI BOT — PAIRING CODE: ${code}`); logger.info(`📱 Número: +${pairingPhone}`); logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`); logger.info(`No WhatsApp, abra Dispositivos conectados e use a opção de conectar por código.`); } catch (error) { pairingRequested = false; logger.error({ err: error }, '❌ Falha ao gerar o Pairing Code.'); } }
    if (connection === 'open') { restarting = false; logger.info(`🤖 ${config.bot.name} conectado com ${commands.size} comandos.`); }
    if (connection === 'close') { const statusCode = lastDisconnect?.error?.output?.statusCode; const shouldReconnect = statusCode !== DisconnectReason.loggedOut; logger.warn(`🔌 Conexão encerrada (${statusCode ?? 'desconhecido'}). Reconectar: ${shouldReconnect}`); if (shouldReconnect && !restarting) { restarting = true; setTimeout(() => startBot().catch(error => { restarting = false; logger.error({ err: error }, '❌ Falha ao reiniciar o Togi Bot'); }), 3000); } }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const message of messages || []) {
      ...
      if (isGroup && !message.key.fromMe && !isAfkToggle) { try { const moderated = await moderateProfanity({ sock, chat, message, sender: effectiveSender }); if (moderated) continue; } catch (error) { logger.debug({ err: error }, 'Falha na moderação anti-palavrão.'); } }
      ...
      if (!text.startsWith(config.bot.prefix) && isTogiActive(chat, effectiveSender)) {
        try {
          ...
          if (error?.name !== 'AbortError') await reply('❌ A Togi AI está indisponível no momento. Tente novamente em instantes.');
        }
        continue;
      }
      ...
      try { const menuImageUrl = isMenuCommand(name) ? await getMenuImageUrl(name) : null; const commandReply = menuImageUrl ? (content, options = {}) => sendMenuReply(sock, chat, message, menuImageUrl, content, options) : reply; await command.execute({ sock, message, sender: effectiveSender, chat, args, text: args.join(' '), rawText: text, commandName: name.toLowerCase(), isGroup, reply: commandReply, react: async (emoji) => { try { await sock.sendMessage(chat, { react: { text: emoji, key: message.key } }); } catch {} } }); } catch (error) { logger.error({ err: error, command: name }, `Erro no comando ${name}`); await reply(`❌ Erro ao executar .${name}: ${error?.message || 'erro desconhecido'}`); }
    }
  });
}
