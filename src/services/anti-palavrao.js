...
export async function moderateProfanity({ sock, chat, message }) {
  ...
  if (!(await isBotAdmin(sock, chat))) return { moderated: false, reason: 'bot-not-admin', word: found };

  const deleteKey = buildDeleteKey(chat, message);
  if (!deleteKey.id) return { moderated: false, reason: 'missing-message-id', word: found };
