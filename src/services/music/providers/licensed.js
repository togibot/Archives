...
function score(query, title, artist = '') {
  ...
  let points = 0;
  if (t === q) points += 100000;
  else if (t.includes(q)) points += 30000;
...
function fmaLicenseAllowed(track) {
  ...
  if (license.includes('noncommercial')) return false;
  if (license.includes('no derivatives') || license.includes('nd')) return false;
...
async function searchFma(query) {
  ...
  const audioUrl = file.startsWith('http') ? file : `https://files.freemusicarchive.org/${file.replace(/^\/+/, '')}`;
  return {
    name: clean(best.track_title),
    artist_name: clean(best.artist_name, 'Artista desconhecido'),
    duration: Number(best.track_duration || best.duration || 0),
    audiodownload: audioUrl,
    source: 'Free Music Archive',
    license: clean(best.track_license || best.license),
    url: clean(best.track_url || best.url)
  };
}
...
export async function searchLicensedTracks(query, originalQuery = query) {
  const [fma, jamendo] = await Promise.all([
    searchFma(query).catch(() => null),
    searchJamendo(query).catch(() => null)
  ]);

  const candidates = [fma, jamendo].filter(Boolean).map(track => ({
    track,
